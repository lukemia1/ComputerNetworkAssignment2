...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (windowcount != 0 && packet.acknum != lastACK) {
...
static int firstr;
static int revfirst; /* the sequence number expected next by the receiver */
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))) 
  {
    ...
    if(data_existed==0)
    {
        revbuffer[revfirst]=packet.seqnum;
        if(revfirst<SEQSPACE)
        {
            revfirst+=1;
        }
